...
def searchItemAtMarket(item):
    ...
    if name is not None:
        ...
        pyautogui.sleep(0.5)
        pyautogui.write(name, interval=0.1)  # type name

    # choose all of filters of item
    if category != None:
        pyautogui.click(xSearchBar + wSearchBar, ySearchBar,
                        duration=0.2)  # click on category list
        pyautogui.sleep(0.7)
        for i, y in enumerate(category):  # move to needly category
            if i == 1:
                pyautogui.sleep(0.5)
            sublist = 225 if i == 1 else 0
            pyautogui.moveTo(xSearchBar + wSearchBar +
                             sublist, y, duration=0.2)
        pyautogui.sleep(0.2)
...
def isAlbionWindowActive():
    while True:
        ...
        img = pyautogui.locateCenterOnScreen(
            "images/cursor.png", confidence=0.9, region=(x-20, y-10, 60, 60))
...
def takeScreenshot(takePosition=1):
    ...
    pyautogui.screenshot("images/t.png", region=(left,
...
def checkColorInRange(
    ...
    for x in range(left, right or left+1):  # range for x coordinate
        for y in range(top, bottom or top+1):  # range for y coordinate
            ...
            if isColorFound:
                foundColorCounter.append(isColorFound)
